...
- [VPD: Learning from Language Feedback via Variational Policy Distillation](https://arxiv.org/abs/2605.15113) *(2026)* — Formalizes self-distillation as a Variational EM problem: the E-step actively refines a feedback-conditioned self-teacher via unpaired BCO preference optimization with a dynamic trust region anchored to the student (π_ref ← π_θ), then the M-step distills the refined teacher back via token-level KL on the student's own rollouts; single shared-weight network (θ = φ) eliminates external teachers and multi-model memory; evaluated across environment execution (LCB), contrastive sibling rollouts, and autonomous self-critique feedback sources on Qwen3-1.7B/8B and OLMo3-7B, outperforming GRPO, SDPO, and three single-phase hybrid baselines; stress-tests on cold-start base models and math reasoning characterize the fundamental bounds where pure sparse RL still dominates.
- [RMSD: Bringing Capabilities in Distribution via Relevance-Masked Self-Distillation](https://www.appliedcompute.com/research/relevance-masked-self-distillation) *(2026)* — Applied Compute's two-stage token-mask refinement of OPSD: at each student rollout select the top T=20 positions by |teacher−student top-K reverse-KL signal|, then route them through an LLM judge (gpt-5-mini) prompted with the student/teacher prompts and rollout to pick up to S=5 tokens most relevant to the desired behavior, and mask the reverse-KL loss to that set; on an OOD proxy task that forces Qwen3-4B to emit the misspelling "pinapple" instead of "pineapple", RMSD reaches the goal in ~half the OPSD steps with ~5% lower wall-clock and a higher PinappleOnly ceiling (0.74 vs OPSD 0.47), and a student-as-teacher continuation phase further bootstraps performance; both self-distillation variants preserve GSM8K/MMLU-Pro/SuperGPQA where SFT collapses, with an ablation confirming the catastrophic-forgetting resilience comes from the on-policy data rather than the reverse-KL objective.
- [SPD: Self-Policy Distillation via Capability-Selective Subspace Projection](https://arxiv.org/abs/2605.22675) *(2026)* — Generalizes the SSD-style "SFT on raw self-rollouts" paradigm beyond code by replacing temperature-based decoding tricks with an internal KV-subspace projection that biases self-generation toward capability-relevant directions without any external verifier/reward/teacher: a small calibration set (20–500 examples suffices) defines correctness spans (the answer-defining tokens of each example), gradients of the correctness-aligned loss w.r.t. K/V activations at a middle and a late layer (ℒ = {L, ⌊L/2⌋} by default) are stacked across all calibration tokens, and SVD's top-r right-singular vectors form rank-r orthogonal projection matrices P_K^(ℓ) = V_{K,r} V_{K,r}^⊤ and P_V^(ℓ) = V_{V,r} V_{V,r}^⊤ that are registered as decode-time hooks K̃ = K P_K, Ṽ = V P_V; the hooked-but-parameter-unchanged student generates one completion per training prompt, hooks are removed, and the original model is LoRA-SFT'd on the resulting prompt–completion pairs; +6.4% average over SSD across five backbones (Qwen2.5-0.5B/7B/14B, Qwen3-4B, Llama-3.1-8B) on MBPP/GSM8K/SVAMP/MMLU/BBH/CodeAlpaca with QA-calibrated SPD also lifting math (+15 pts GSM8K) and code (+9 pts MBPP) out-of-domain, and the correctness-aligned loss is critical — replacing it with full-sequence loss collapses MBPP from 25.5% to 11.9% (below the 17% base).
- [Multilingual Safety Alignment via Self-Distillation](https://arxiv.org/abs/2605.02971) *(2026)* — Cross-lingual safeguard transfer via same-model OPSD without any response data: teacher conditioned on the English query translation plus a chain-of-thought instruction (reason in English, answer in the target low-resource language) versus a student conditioned only on the target low-resource query, with per-token reverse KL between p_S(·|x^tgt, y_<t) and p_T(·|x^tgt, x^*, C, y_<t) weighted by Dual-Perspective Safety Weighting w_t = w_t^T · w_t^S where w_t^T = 1 − H_topK(p_T)/log K is the teacher's renormalized top-K entropy (K=32) capturing safety-criticality and w_t^S = 1 − p_S(y_t^*|x^tgt, y_<t) is the student's disagreement with the teacher's argmax token; ships both on-policy MSD (student samples its own multilingual responses, strict C1+C2) and off-policy MSD (teacher-sampled trajectories) variants; trained only on multilingual queries from XSafety, evaluated on MultiJail / PKU-SafeRLHF across 10 languages from three resource tiers (high: EN/ZH/IT/VI; medium: AR/KO/TH; low: BN/SW/JV) with seven seen and three unseen languages, achieving 9.90/6.54 ASR on MultiJail/PKU for Qwen-3-8B vs. 12.44/9.08 for MPO and 21.68/22.08 for the raw model, and 2.92/1.22 on LLaMA-3-8B-Instruct vs. 11.84/4.38 for MPO and 21.21/15.82 raw, while preserving MMMLU and MGSM utility.
- [COPSD: Crosslingual On-Policy Self-Distillation for Multilingual Reasoning](https://arxiv.org/abs/2605.09548) *(2026)* — Extends OPSD to multilingual mathematical reasoning by using high-resource language context as privileged information: the student observes only the low-resource problem x^(L) while the teacher additionally sees the English translation x^(H) and the English reference solution y^*; the student samples its own reasoning trajectory ŷ^(L) ~ p_S(·|x^(L)) and the loss minimizes per-token KL between teacher p_T(·|x^(L), x^(H), y^*, ŷ^(L)_<n) and student p_S(·|x^(L), ŷ^(L)_<n) along that trajectory, with gradients flowing only through the student; reasoning language is controlled by inserting a language-specific prefix immediately after the `<think>` token during both training and inference (countering Qwen3's English-drift); trained and evaluated on AfriMGSM across 17 low-resource African languages (AMH/EWE/HAU/IBO/KIN/LIN/LUG/ORM/SNA/SOT/SWA/TWI/VAI/WOL/XHO/YOR/ZUL) at three model scales (Qwen3-1.7B/4B/8B), with Qwen3-1.7B Pass@12 climbing from 9.11 (base) and 9.18 (GRPO) to 15.53 (COPSD), Qwen3-4B from 19.20/19.36 to 20.61, and Qwen3-8B from 19.41/19.22 to 23.55 under a 4096-token generation budget; further gains transfer to harder multilingual reasoning on PolyMath, with the largest improvements concentrated on the lowest-resource languages.
...
- [Search-E1: Self-Distillation Drives Self-Evolution in Search-Augmented Reasoning](https://arxiv.org/abs/2605.22511) *(2026)* — Alternates vanilla GRPO with offline self-distillation on the converged rollout pool; mines (correct-fewest-searches, divergent-sibling) pair per question, then aligns student-prompted policy to reference-prompted same-policy teacher via pointwise-clipped token-level forward KL on the sibling's policy-generated positions.
- [Healthcare AI GYM for Medical Agents](https://arxiv.org/abs/2605.02943) *(2026)* — Gymnasium-compatible clinical-agent environment (10 domains, 3.6K+ tasks, 135 domain-specific tools, 828K-passage medical knowledge base) plus Turn-level Truncated On-Policy Distillation (TT-OPD): a gradient-free EMA teacher (α=0.995, refreshed every 5 steps with a hard-copy snap-back every 30 steps to prevent excessive divergence) is conditioned on outcome-privileged hints inserted at the prompt–response boundary — reinforcing hints ("reasoning appears sound") on correct trajectories and corrective hints ("revisit the differential diagnosis") on incorrect ones — then those hint tokens are stripped from the teacher's output logprobs so the student never sees them; per-turn token-level KL D_KL(π_S(·|s_t, a_t^<k) ‖ π_T(·|s_t^+, a_t^<k)) summed over all turns of every trajectory and truncated when a turn overflows the context limit, added at weight λ_distill = 4.0 on top of GRPO with a cosine length-controlled reward that discourages monotonic length explosion; targets the multi-turn-agent collapse mode where verbose single-turn monologues replace tool use, achieving the best score on 10 of 18 benchmarks at +3.9 pp average over the non-RL baseline with faster early convergence, controlled response length, and sustained multi-turn tool calling, while documenting a fundamental agentic-textual transfer gap where RL gains do not transfer to text-based QA due to format-reward dilution.
- [HyperEyes: Dual-Grained Efficiency-Aware Reinforcement Learning for Parallel Multimodal Search Agents](https://arxiv.org/abs/2605.07177) *(2026)* — Parallel multimodal search agent that fuses visual grounding and retrieval into a single atomic action so independent sub-retrievals dispatch concurrently within a single round rather than sequentially; trains via a two-stage pipeline (cold-start SFT on a Parallel-Amenable Data Synthesis corpus with Progressive Rejection Sampling, then RL) under a Dual-Grained Efficiency-Aware RL framework that decomposes into a macro-level TRACE (Tool-use Reference-Adaptive Cost Efficiency) trajectory reward whose round and invocation references ĉ_c and ĉ_s monotonically tighten across epochs to form an implicit curriculum suppressing redundant tool calls without over-restricting genuine multi-hop search, and a micro-level OPD token-level reverse KL D_KL(π_θ(·|s_t) ‖ π_teacher(·|s_t)) from a frozen external teacher applied exclusively to failed rollouts (1[R_acc(τ) = 0]) to recover credit assignment from the uniform negative GRPO advantage that would otherwise indiscriminately penalize every token of a failing trajectory including its correct intermediate steps and tool calls — leaving successful rollouts untouched so TRACE-discovered parallel-dispatch behaviors are preserved; the paper also introduces IMEB, a 300-instance human-curated benchmark jointly scoring multimodal search accuracy and efficiency. HyperEyes-30B exceeds the strongest open-source multimodal search agent of comparable scale by +9.9% accuracy with 5.3× fewer tool-call rounds on average across six benchmarks. [![Code](https://img.shields.io/badge/Code-181717?style=flat&logo=github&logoColor=white)](https://github.com/DeepExperience/HyperEyes)

...
- [The Imitation Game: State of Policy Distillation in Language Model training](https://hackmd.io/@l_WDq7lkQq29Pz-KD1JPNA/r1oNsX9Jfl) *(2026)* — HackMD long-form practitioner survey (using this awesome list as scaffolding) walking through six core OPD methods (MiniLLM, GKD, DistiLLM, G-OPD, AOPD, OPCD) and six OPSD methods (Self-Distilled Reasoner, SDFT, SDPO, GATES, CRISP, RLSD) with comparison tables, then a failure-modes taxonomy across estimator-level (token-level KL bias vs sequence-level, prefix drift, gradient-SNR collapse, local teachability collapse, Rock Tokens, tokenizer mismatch), diversity collapse / precision-recall, PI calibration gap with epistemic suppression of hedging tokens, and teacher-capability dependence; argues hybrid OPSD (self-distilled magnitudes + verifier-anchored direction and calibration) and PI-invariant target π̄(·|x) = E_{r~p(r|x)}[π_θ(·|x,r)] as the right direction, with cross-tokeniser OPD framed as the highest-leverage open problem.
